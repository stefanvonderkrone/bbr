...
All remote TUI work uses one TUI-lifetime `StdHttpClient` connection pool. Candidate Session acquisition uses at most two active requests. PullRequest, RawDiff, and Comments are required. Authenticated Account acquisition is independent. Comments wait for PullRequest commit data and take priority over RawDiff when both can start. Remote File Enrichment fetches its old and new sides concurrently, with at most two requests per File. All started branches complete without branch cancellation. LocalReview acquisition remains sequential. LocalReview File Enrichment also remains sequential.
