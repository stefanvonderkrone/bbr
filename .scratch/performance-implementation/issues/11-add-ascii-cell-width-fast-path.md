...
The 12.21-second baseline Time Profiler trace (`cell-width-baseline-time.trace`) shows vaxis grapheme iteration and `gwidth` in the hot stack. The 10.26-second changed Time Profiler trace (`cell-width-changed-time.trace`) shows `isNarrowAscii` as the remaining measured work. The baseline Allocations trace (`cell-width-baseline-allocations.trace`), changed Allocations trace (`cell-width-changed-allocations.trace`), baseline CPU Counters trace (`cell-width-baseline-counters.trace`), and changed CPU Counters trace (`cell-width-changed-counters.trace`) completed without errors. The harness classified the stage as instruction-throughput work. Its gap changed from 92.61% at baseline to 0% after the change, but this rate is a calibration proxy.
