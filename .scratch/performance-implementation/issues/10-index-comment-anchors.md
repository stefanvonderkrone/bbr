...
The 13.42-second baseline Time Profiler trace (`comment-anchors-baseline-time.trace`) shows `weaveInline`, `anchorMatchesFile`, `anchorMatchesLine`, and `disclosureExpanded` in the hot stack. The 12.83-second changed Time Profiler trace (`comment-anchors-changed-time.trace`) used 1,200 repetitions. Hash lookup, ReviewCard projection, and checksum work remain. The baseline CPU Counters trace (`comment-anchors-baseline-counters.trace`) and changed CPU Counters trace (`comment-anchors-changed-counters.trace`) provide hardware evidence for an instruction-throughput workload. The harness reports an 86.96% instruction-rate gap for the final 2,000-item run, but this rate is a calibration proxy rather than a hardware limit.
